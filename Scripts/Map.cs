...
{
    //declare grid types
    public static int GRIDTYPE_OWN_INVENTORY = 0;//self prepare zone
    public static int GRIDTYPE_OPONENT_INVENTORY = 1;//other prepare zone
    public static int GRIDTYPE_HEXA_MAP = 2;//combat zone
    ...
    public Transform mapOponentStartPosition;
    ...
    private void CreateGridPosition()
    {
        ...
        //single player mode or master player start at positive side
        ...
        //slave player start at negative side
        else
        {
            //create own inventory position
            for (int i = 0; i < inventorySize; i++)
            ...
            for (int x = 0; x < hexMapSizeX; x++)
            {
                for (int z = 0; z < hexMapSizeZ; z++)
                {
                    ...
                    //calculate and store the position
                    Vector3 position = GetMapHitPoint(mapOponentStartPosition.position + new Vector3(offsetX, 0, offsetZ));
